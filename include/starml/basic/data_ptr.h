...
// A detail::DataPtr is an owning smart pointer like unique_ptr, but
...
class DataPtr {
 ...
 public:
  DataPtr() : data_(nullptr), ctx_(nullptr, &delete_nothing) {}
  explicit DataPtr(void* data) : data_(data), ctx_(nullptr, &delete_nothing) {}
  ...
  bool compare_exchange_deleter(DeleterFnPtr expected_deleter,
...
// Note [How DataPtr is implemented]
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
// DataPtr solves a common problem for allocators of tensor data, which
...
inline bool operator==(const DataPtr& sp, std::nullptr_t) noexcept {
  return !sp;
}
inline bool operator==(std::nullptr_t, const DataPtr& sp) noexcept {
  return !sp;
}
inline bool operator!=(const DataPtr& sp, std::nullptr_t) noexcept {
  return sp;
