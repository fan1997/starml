...
namespace starml {
namespace {
void copy_gpu_to_cpu_impl(const Matrix& src, Matrix& dst, void* stream) {
...
void copy_cpu_to_gpu_impl(const Matrix& src, Matrix& dst, void* stream) {
  auto dtype = src.data_type().type();
  STARML_DISPATCH_TYPES(dtype, "CUDA_COPY", [&]() {
    auto src_data = src.data<scalar_t>();
    auto dst_data = dst.mutable_data<scalar_t>();
...
void copy_gpu_to_gpu_impl(const Matrix& src, Matrix& dst, void* stream) {
}
}  // namespace
