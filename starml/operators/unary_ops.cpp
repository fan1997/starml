...
Matrix log(const Matrix& matrix, Handle* handle) {
  // If the input matrix is int, then the result data type should be float
  // else the data type of result should be consistent with the input
  auto result_dtype =
      (matrix.data_type().is_int()) ? kFloat : matrix.data_type().type();
  Matrix result = Matrix(matrix.dims(), matrix.device(), result_dtype);
  log_dispatcher(matrix, result, handle);
  return result;
}
Matrix log(const Matrix& matrix, Matrix& result, Handle* handle) {
  log_dispatcher(matrix, result, handle);
  return result;
}


Matrix negtive(const Matrix& matrix, Handle* handle) {
  Matrix result = Matrix(matrix.dims(), matrix.device(),  matrix.data_type());
  negtive_dispatcher(matrix, result, handle);
  return result;
}
Matrix negtive(const Matrix& matrix, Matrix& result, Handle* handle) {
  negtive_dispatcher(matrix, result, handle);
  return result;
}
