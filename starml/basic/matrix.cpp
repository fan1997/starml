...
Matrix::Matrix(Shape shape, Device device, DataType data_type)
    : shape_(shape), device_(device), dtype_(data_type) {
  initial();
}
Matrix::Matrix(Shape shape, DataType data_type, Device device)
...
const Device& Matrix::mutable_device() const { return this->device_; }
Device Matrix::device() const { return this->device_; }
